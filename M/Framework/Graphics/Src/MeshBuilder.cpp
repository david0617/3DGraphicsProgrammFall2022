...
Mesh MeshBuilder::CreatePlane(int numRows, int numColumns, float spacing)
{
    ...
    const MMath::Vector3 n = MMath::Vector3::YAxis;
    const MMath::Vector3 t = MMath::Vector3::XAxis;
    for (int r = 0; r <= numRows; ++r)
    {
        ...
        for (int c = 0; c <= numColumns; ++c)
        {
            ...
            mesh.verticies.push_back({ {x, 0.0f , z  }, n, t, MMath::Vector2(u,v) });
            x += spacing;
        }
        x = -halfPlaneWidth;
        z += spacing;
...
MeshPC MeshBuilder::CreateCylinderPC(int slices, int rings)
{
    ...
    for (int  r = 0; r <= rings; ++r)
    {
        ...
        float phi = r * (MMath::Constants::Pi/static_cast<float>(rings));


        for (int s = 0; s < slices + 1; ++s)
        {
            float slice = static_cast<float>(s);
            float rotations = (slice / static_cast<float>(slices)) * MMath::Constants::TwoPi;
...
MeshPC MeshBuilder::CreateSpherePC(int slices, int rings, float radius)
{
    ...
    {
        float ring = static_cast<float>(r);
        float phi = ring * (MMath::Constants::Pi / static_cast<float>(rings - 1));

        for (int s = 0; s < slices + 1; ++s)
        {
            float slice = static_cast<float>(s);
            float rotation = (slice / static_cast<float>(slices)) * MMath::Constants::TwoPi;
...
Mesh MeshBuilder::CreateSphere(int slices, int rings, float radius)
{
    ...
    for (int r = 0; r <= rings; ++r)
    {
        ...
        for (int s = 0; s < slices + 1; ++s)
        {
            ...
            const MMath::Vector3 pos = {x, y, z};
            const MMath::Vector3 norm = MMath::Normalize(pos);
            const MMath::Vector3 tan = MMath::Normalize({-z, 0.0f, x});
